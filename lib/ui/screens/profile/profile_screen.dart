...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      if (p.statusCode >= 200 && p.statusCode < 300) {
        final list = (jsonDecode(p.body) as List?) ?? [];
        _projectOptions = list
            .map((e) => _ProjectOption(
                  code: (e['project_code'] ?? '').toString(),
                  name: (e['project_name'] ?? '').toString(),
                ))
            .where((x) => x.code.isNotEmpty && x.name.isNotEmpty)
      ...
      // User profile
      final up = await _api.get('/api/user-profile/${Uri.encodeComponent(_emailCtrl.text)}');
      if (up.statusCode == 404) {
        _isNewProfile = true;
        _designation = _designationOptions.first;
        _avatarNetwork = null;
        ...
        // tell session there is no avatar (so app bars show placeholder)
        await context.read<UserSession>().setAvatarUrl(null);
      } else if (up.statusCode >= 200 && up.statusCode < 300) {
        _isNewProfile = false;
        final body = jsonDecode(up.body) as Map<String, dynamic>;
        final prof = (body['user_profile'] as Map?) ?? {};
        ...
        // push avatar into session (cache-busted)
        final busted = _avatarNetwork == null
            ? null
            : '${_avatarNetwork!}?t=${DateTime.now().millisecondsSinceEpoch}';
        await context.read<UserSession>().setAvatarUrl(busted);
      } else {
        _toast('Failed to load profile');
      }
    } catch (_) {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final files = <http.MultipartFile>[];
      if (_avatarFile != null) {
        files.add(await http.MultipartFile.fromPath('profile_picture', _avatarFile!.path));
      }

      final path = _isNewProfile
          ? '/api/user-profile'
          : '/api/user-profile/${Uri.encodeComponent(email)}';
      final method = _isNewProfile ? 'POST' : 'PUT';
      ...
      if (pic.isNotEmpty) {
        ...
        // update app-wide avatar immediately (bust cache like web)
        final busted = '${_avatarNetwork!}?t=${DateTime.now().millisecondsSinceEpoch}';
        await context.read<UserSession>().setAvatarUrl(busted);
      } else {
        await context.read<UserSession>().setAvatarUrl(null);
      }
                      ...
                      child: Column(
                        ...
                        children: [
                              ...
                              child: CircleAvatar(
                                ...
                                child: CircleAvatar(
                                  ...
                                  backgroundImage: _avatarFile != null
                                      ? FileImage(_avatarFile!)
                                      : (_avatarNetwork != null
                                          ? NetworkImage(_avatarNetwork!) as ImageProvider
                          ...
                          CustomTextField(
                            ...
                            showAction: true,
                            actionLabel: _editingName ? 'SAVE' : 'EDIT',
                            onActionTap: () => setState(() => _editingName = !_editingName),
                          ),
                          const SizedBox(height: 16),
                          ...
                          CustomTextField(
                            ...
                            showAction: true,
                            actionLabel: _editingContact ? 'SAVE' : 'EDIT',
                            onActionTap: () => setState(() => _editingContact = !_editingContact),
                          ),
                          const SizedBox(height: 24),
                          ...
                          LayoutBuilder(
                            builder: (context, c) {
                              ...
                              return narrow
                                  ? Column(children: [username, SizedBox(height: gap), designation])
                                  : Row(children: [Expanded(child: username), SizedBox(width: gap), Expanded(child: designation)]);
                            },
                          ),
                          const SizedBox(height: 16),
                          ...
                          LayoutBuilder(
                            builder: (context, c) {
                              ...
                              final projects = _ProjectsPicker(
                                pickedNames: _projectOptions
                                    .where((p) => _selectedProjectCodes.contains(p.code))
                                    .map((p) => p.name)
                                    .toList(),
                                onTap: _pickProjects,
                              );
                              ...
                              return narrow
                                  ? Column(children: [role, SizedBox(height: gap), projects])
                                  : Row(children: [Expanded(child: role), SizedBox(width: gap), Expanded(child: projects)]);
                            },
                          ),
                          const SizedBox(height: 16),

                          // Email
                          CustomTextField(label: 'Email', controller: _emailCtrl, readOnly: true),
                          ...
                          Align(
                            alignment: Alignment.centerRight,
                            child: ConstrainedBox(
                              constraints: BoxConstraints(maxWidth: context.w < 360 ? 120 : 160),
                              child: 
                              // GradientButton(
                              //   text: _busy ? 'Saving…' : 'Save',
                              //   onPressed: _busy ? null : () async => await _save(),
                              // ),
                                GradientButton(
                                  text: _busy ? 'Saving…' : 'Save',
                                  onPressed:
