...
class _AddUserScreenState extends State<AddUserScreen> {
  ...
  // NOTE: match web (string values). We'll trim before sending.
  final _roles = const ['Admin', 'Project Manager', 'NOC ', 'SCM'];
  String? _role;
  ...
  Future<void> _createUser() async {
    if (!_valid) {
      if (!_agree) {
        _toast('You must agree to Terms & Conditions');
    ...
    try {
      final body = {
        ...
        'role': (_role ?? '').trim(), // trim to avoid trailing space from 'NOC '
      };

      final res = await _api.post('/api/auth/signup/user', body: body);
                    ...
                    children: [
                      ...
                      LayoutBuilder(
                        builder: (context, c) {
                          ...
                          final left = [
                            _TextField(label: 'Username *', controller: _usernameCtrl),
                            _TextField(
                              label: 'Email ID *',
                              controller: _emailCtrl,
                              keyboardType: TextInputType.emailAddress,
...
class _PasswordField extends StatelessWidget {
  ...
  final bool visible;
  final VoidCallback onToggle;
  const _PasswordField({required this.label, required this.controller, required this.visible, required this.onToggle});
    ...
    return _FieldShell(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          suffixIcon: IconButton(
            onPressed: onToggle,
            icon: Icon(visible ? Icons.visibility_off : Icons.visibility, color: cs.onSurfaceVariant),
          ),
        ),
      ),
    );
